...
describe("catalogBatchProcess", () => {
	...
	beforeEach(async () => {
		...
		catalogBatchProcess = (await import("../lambda-functions/catalogBatchProcess")).handler;
	});
	...
	it("should handle DynamoDB error", async () => {
		...
		ddbDocClientMock.on(TransactWriteCommand).rejects(new Error("DynamoDB error"));
		snsClientMock.on(PublishCommand).resolves({});

		await expect(catalogBatchProcess(sqsEvent)).rejects.toThrow("DynamoDB error");
	});
	...
	it("should handle SNS publish error", async () => {
		...
		await expect(catalogBatchProcess(sqsEvent)).rejects.toThrow("SNS publish error");
	});
});
